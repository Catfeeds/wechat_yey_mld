CREATE TABLE `student_onboard_checkingin` (
  ...
  `owner_id` mediumint(8) unsigned NOT NULL DEFAULT '0' COMMENT '考勤人',
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `student_onboard_checkingin_detail_view` AS select `student_onboard_checkingin_detail`.`id` AS `id`,`student_onboard_checkingin_detail`.`check_id` AS `check_id`,`student_onboard_checkingin_class_view`.`date` AS `date`,`student_onboard_checkingin_class_view`.`active` AS `active`,`student_onboard_checkingin_class_view`.`class_id` AS `class_id`,`student_onboard_checkingin_class_view`.`class_name` AS `class_name`,`student_onboard_checkingin_detail`.`student_id` AS `student_id`,`student_onboard_info`.`name` AS `name`,`student_onboard_checkingin_detail`.`type` AS `type`,`student_onboard_checkingin_detail`.`comment` AS `comment` from ((`student_onboard_checkingin_detail` join `student_onboard_info` on((`student_onboard_checkingin_detail`.`student_id` = `student_onboard_info`.`student_id`))) join `student_onboard_checkingin_class_view` on((`student_onboard_checkingin_class_view`.`id` = `student_onboard_checkingin_detail`.`check_id`)));
