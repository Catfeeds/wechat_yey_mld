...
$o_stu=new Student_Info_Wechat_View($_GET['student_id']);
if ($o_stu->getName()=='' || $o_stu->getName()==null || $o_stu->getUserId()!=$o_wx_user->getId())
{
...
?>
	...
	<form action="include/bn_submit.switch.php" id="submit_form" method="post" target="ajax_submit_frame" onsubmit="this.submit()">
		...
		<input type="hidden" name="Vcl_FunName" value="StudentModify"/>
		<input type="hidden" name="Vcl_CourseId" value="<?php echo($_GET['course_id'])?>"/>
		<input type="hidden" name="Vcl_Id" value="<?php echo($_GET['student_id'])?>"/>
	    <div class="weui-cells" style="margin-top:0px;">
	        <div class="weui-cell">
	            <div class="weui-cell__bd">
	                <input disabled="disabled" id="Vcl_Name" name="Vcl_Name" style="color:#000000" class="weui-input" type="text" placeholder="学生姓名"/>
	            </div>
	        </div>
	        <div class="weui-cell weui-cell_select weui-cell_select-after">
	            ...
	            <div class="weui-cell__bd">
	                <select class="weui-select" id="Vcl_IdType" name="Vcl_IdType" onchange="student_change_idtype(this)" disabled="disabled">
	        ...
	        <div class="weui-cell">
	            <div class="weui-cell__bd">
	                <input class="weui-input" type="text" placeholder="证件号码" id="Vcl_IdCard" name="Vcl_IdCard" style="color:#000000" onBlur="check_id()" disabled="disabled"/>
	            </div>
	        </div>
	        ...
	        <div id="harea" class="weui-cell weui-cell_select weui-cell_select-after" style="display:none">
	            <div class="weui-cell__hd">
	    ...
	    <div style="padding:15px;">
		    <a class="weui-btn weui-btn_primary" onclick="sutdent_add()">保存</a>
	    </div>
...
<script>
$(function () {
	$('#Vcl_Name').val('<?php echo($o_stu->getName())?>');
	$('#Vcl_IdType').val('<?php echo($o_stu->getIdType())?>');
	$('#Vcl_IdCard').val('<?php echo($o_stu->getIdCard())?>');
	$('#Vcl_Sex').val('<?php echo($o_stu->getSex())?>');
	$('#Vcl_Birthday').val('<?php echo($o_stu->getBirthday())?>');
	$('#Vcl_Nation').val('<?php echo($o_stu->getNation())?>');
	$('#Vcl_Phone').val('<?php echo($o_stu->getPhone())?>');
	$('#Vcl_ParentName').val('<?php echo($o_stu->getParentName())?>');
	$('#Vcl_HArea').val('<?php echo($o_stu->getHArea())?>');
	$('#Vcl_XArea').val('<?php echo($o_stu->getXArea())?>');
}); 
//禁止分享
