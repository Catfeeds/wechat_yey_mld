$(function(){
	...
	$('#Vcl_KeyTeacherYeInfo').keypress(function(event){  
	    var keycode = (event.keyCode ? event.keyCode : event.which);  
	    if(keycode == '13'){  
	    	search_for_teacher_yeinfo()   
	    }  
	});
	$('#Vcl_KeyYeGraduate').keypress(function(event){  
	    var keycode = (event.keyCode ? event.keyCode : event.which);  
	    if(keycode == '13'){  
...
{
	var fun='TeacherYeInfo';
	var id='Vcl_KeyTeacherYeInfo'
	$('.small_loading').fadeIn(100);
	$.cookie(fun+"Page",1);
	$.cookie(fun+"OtherKey",document.getElementById(id).value);
	var sort=$.cookie(fun+"Sort"); 
...
function search_for_yegraduate()
{
	var fun='YeGraduateTable';
	var id='Vcl_KeyYeGraduate'
	$('.small_loading').fadeIn(100);
	$.cookie(fun+"Page",1);
	$.cookie(fun+"OtherKey",document.getElementById(id).value);
	var sort=$.cookie(fun+"Sort"); 
...
function stu_unbinding(id) {
    dialog_confirm('真的要解除幼儿绑定的微信账户吗？解绑后不能恢复，请谨慎操作。',function(){
    	$('.small_loading').fadeIn(100);
    	var data = 'Ajax_FunName=StuUnbinding'; //后台方法
        data = data + '&id=' + id;
        $.getJSON("include/bn_submit.switch.php", data, function (json) {
        	if (json.success==0)
        	{
...
function stu_cancel_graduate(id,name,class_name) {
	var a_arr=[];
	a_arr.push('<select name="Vcl_ClassId" id="Vcl_ClassId" class="form-control" style="width:auto">');
	a_arr.push('<option value="">请选择班级</option>');
	for(var i=0;i<a_class_list.length;i++)
	...
	a_arr.push('</select>');
    dialog_confirm('幼儿姓名：<b>'+name+'</b><br/>原始班级名称：<b>'+class_name+'</b><br/><br/>请选择班级后，请按确认按钮完成撤销毕业。<br/><br/>'+a_arr.join('\n'),function(){
    	$('.small_loading').fadeIn(100);
    	var data = 'Ajax_FunName=StuCancelGraduate'; //后台方法
        data = data + '&id=' + id+'&classid='+$('#Vcl_ClassId').val();
        $.getJSON("include/bn_submit.switch.php", data, function (json) {
        	if (json.success==0)
        	{
				...
				$('.small_loading').fadeOut(100);
				dialog_success('撤销毕业成功！',function(){
					table_refresh('YeGraduateTable')
				})
        	}        	
        })
    })
...
function ye_checkingin_makeup_submit()
{
	dialog_confirm("真的要提交补录考勤记录吗？<br/>提交后不能修改，请谨慎操作！",function (){
		loading_show();
		$('#submit_form').submit();
	});
}
