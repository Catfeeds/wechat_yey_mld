...
通知类型：微视频');
			...
			$o_msg->setKeyword5('');
			$o_msg->setRemark('');
			$o_msg->setUrl($o_system_setup->getHomeUrl().'sub/wechat/teacher_operation/wei_teach_review.php?id='.$this->getPost('Id'));
			$o_msg->setKeywordSum(10);
			$o_msg->Save();
		}
...
通知类型：微海报');
			...
			$o_msg->setKeyword5('');
			$o_msg->setRemark('');
			$o_msg->setUrl($o_system_setup->getHomeUrl().'sub/wechat/teacher_operation/teaching_h5_review.php?id='.$this->getPost('Id'));
			$o_msg->setKeywordSum(10);
			$o_msg->Save();
		}
		$this->setReturn ( 'parent.form_return("dialog_success(\'发布成功！\',function(){\\parent.location=\''.$this->getPost('BackUrl').'\'})");' );	
	}
	public function NewsTable($n_uid)
	{
		$this->N_PageSize= 50;
		if (! ($n_uid > 0)) {
			$this->setReturn('parent.goto_login()');
		...
		for($i = 0; $i < $n_count; $i ++) {
			...
			if($o_user->getState($i)==1)
			{
				...
				array_push ( $a_button, array ('编辑新闻', "location='news_list.php?id=".$o_user->getId($i)."'" ) );
				array_push ( $a_button, array ('发布', "location='news_release.php?id=".$o_user->getId($i)."'" ) );
				array_push ( $a_button, array ('删除', "news_delete(".$o_user->getId($i).")" ) );
			}
			array_push ($a_row, array (
					($i+1+$this->N_PageSize*($n_page-1)),
					str_replace(' ', '<br/>', $o_user->getCreateDate ( $i )),
		...
		$a_title=$this->setTableTitle($a_title,'当前状态', 'State', 0, 60);
		$a_title=$this->setTableTitle($a_title,Text::Key('Operation'), '', 0,70);
		$this->SendJsonResultForTable($n_allcount,'NewsTable', 'yes', $n_page, $a_title, $a_row);
	}
	public function NewsAdd($n_uid)
	{
		sleep(1);
		if (! ($n_uid > 0)) {
			$this->setReturn('parent.goto_login()');
	...
	public function NewsDelete($n_uid) {
		if (! ($n_uid > 0)) {
			$this->setReturn('parent.goto_login()');
		}
		$o_user = new Single_User ( $n_uid );
		if (! $o_user->ValidModule ( 120503 ))return; //如果没有权限，不返回任何值
		$o_table = new Teaching_News ($this->getPost('id'));
		$o_table->Deletion();
		$o_table=new Teaching_News_List();
		$o_table->PushWhere ( array ('||', 'NewsId', '=',$this->getPost('id')) );
		$o_table->DeletionWhere();
		$a_general = array (
				'success' => 1,
				'text' =>''
		);
	...
	public function NewsModify($n_uid)
	{
		sleep(1);
		if (! ($n_uid > 0)) {
			$this->setReturn('parent.goto_login()');
